...
export const Category = ({ categories, userId, category }: Props) => {
	...
	const handleSubmit = async () => {
		try {
			...
			// return router.push(`/all-listings/new/${res?.listing?._id}/location`);
		} catch (error) {
			setLoading(false);
			...
			<div className="container max-w-3xl">
				...
				{categories?.length !== 0 && (
					<div className="mt-4 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2 md:gap-4">
						{categories?.map(
							({ image, name, _id }: any, index: string) => (
								<div
									key={index}
									className={cn(
